...
export const Login = () => {
    ...
    useEffect(() => {
        if (info) {
            if (info.data.success == 1) {
                localStorage.setItem("session_uid", JSON.stringify(info.data.Id))
                navigate("/")
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit } = useFormik({
        ...
        onSubmit: async (data, action) => {
            console.log(data)
            setLoader(true)
            await axios.post("https://sparkstoideas.daddy11.in/user/login", { phno: data.phone, password: data.password })
                        ...
                        <div className='w-full max-w-sm px-3'>
                            <div className='p-4 w-full rounded-md  bg-[#ffffff8c] space-y-8'>
                                <div className='flex rounded-md w-full bg-white hover:cursor-pointer'>
                                    <div className='w-full bg-[#6EDA48] text-center text-white font-medium p-2 rounded-md'>User</div>
                                    <div onClick={() => navigate("/v_login")} className='w-full text-center p-2 rounded-md font-medium'>Vendor</div>
                                </div>
                                ...
                                <div className='text-sm font-semibold text-center'>Don’t have an account?<span onClick={() => navigate('/register')} className='text-[#6EDA48] hover:cursor-pointer'> Register</span></div>
                            </div>
                        </div>
                    </div>
                </div >
