...
export const VendorList = () => {
  ...
  const action = async (id) => {
    setLoader(true);
    await axios
      .get(`https://sparkstoideas.daddy11.in/vender/loginstatus/${id}`)
      .then((res) => {
  ...
  const del = async (id) => {
    setLoader(true);
    await axios
      .delete(`https://sparkstoideas.daddy11.in/vender/deleteVender/${id}`)
      .then((res) => {
        console.log(res);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (e) => (
        <button
          onClick={() => navigate("/admin/vehicle", { state: e._id })}
